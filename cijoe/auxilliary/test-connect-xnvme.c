 ...
 * gcc test-connect-xnvme.c -o test-connect-xnvme -lhomic -lxnvme; ./test-connect-xnvme 0000:01:00.0 upcie 67 [lba]
 */
#include <errno.h>
#include <inttypes.h>
#include <stdio.h>
...
static int
test_xnvme(const char *dev_uri, const char *be, uint64_t shm_id, uint64_t lba, int do_read)
{
	...
	xnvme_opts.be = be;
	xnvme_opts.shm_id = shm_id;
	xnvme_opts.nsid = 1;

	err = homic_connect_xnvme((char *)dev_uri, &xnvme_opts, &dev);
	if (err) {
...
{
	uint64_t lba = 0;
	int do_read = 0, shm_id = 0, err = 0;

	if (argc < 4) {
		printf("Usage: %s <dev_uri> <be> <shm_id> [lba]\n", argv[0]);
		return 1;
	}

	shm_id = strtol(argv[3], (char **)NULL, 10);

	if (argc > 4) {
		lba = strtoull(argv[4], NULL, 0);
		do_read = 1;
	}
	...
	err = test_xnvme(argv[1], argv[2], shm_id, lba, do_read);
